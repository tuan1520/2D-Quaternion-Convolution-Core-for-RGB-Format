...
module demo_slow_clk_gen #(
    parameter int HALF_COUNT = 5_000_000
)(
    input  logic i_clk,
    input  logic i_rst_n,
    output logic o_clk_slow
);
    localparam int CW = (HALF_COUNT <= 1) ? 1 : $clog2(HALF_COUNT);
    localparam logic [CW-1:0] HALF_LAST = HALF_COUNT - 1;
...
module demo_toggle_pulse_rx (
    input  logic i_clk,
    input  logic i_rst_n,
    input  logic i_toggle_async,
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            ...
            sync_ff1   <= i_toggle_async;
            sync_ff2   <= sync_ff1;
            sync_ff2_d <= sync_ff2;
            o_pulse    <= sync_ff2 ^ sync_ff2_d;
        end
    end
endmodule
...
module demo_image_rom_16x16 #(
    ...
    initial begin
        //$readmemh("testcases/simple_testcases/testcase3/image_16x16_rgb.hex", mem);
        $readmemh("testcases/complex_testcases/testcase2/image_16x16_rgb.hex", mem);
    end
...
module demo_kernel_loader (
    ...
    initial begin
        //$readmemh("testcases/simple_testcases/testcase3/kernel_3x3.hex", ker_mem);
        $readmemh("testcases/complex_testcases/testcase2/kernel_3x3.hex", ker_mem);
    end
...
module demo_pixel_feeder (
    ...
    assign launch_now = active & ~done_now & i_tb_ready;
...
module demo_output_capture #(
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        ...
        else if (i_write_en && wr_addr_lt_depth) begin
...
module da1_demo1 (
    ...
    logic start_toggle_a;
    logic next_toggle_a;
    logic prev_toggle_a;

    logic start_pulse_a_core;
    logic next_pulse_a_core;
    logic prev_pulse_a_core;

    logic start_evt_core;
    logic next_evt_core;
    logic prev_evt_core;

    logic start_frame_cmd;
    logic next_frame_cmd;
    logic run_frame_cmd;
    ...
    demo_slow_clk_gen #(
        .HALF_COUNT(5_000_000)
    ) u_slow_clk_gen (
        .i_clk     (CLOCK_50),
        .i_rst_n   (rst_n_sync),
        .o_clk_slow(slow_clk_5hz)
    );

    assign core_clk = mode_fullspeed ? CLOCK_50 : slow_clk_5hz;

    demo_toggle_pulse_tx u_start_tx (
        .i_clk   (CLOCK_50),
        .i_rst_n (rst_n_sync),
        .i_pulse (start_pulse & ~mode_fullspeed),
        .o_toggle(start_toggle_a)
    );

    demo_toggle_pulse_tx u_next_tx (
        .i_clk   (CLOCK_50),
        .i_rst_n (rst_n_sync),
        .i_pulse (next_pulse & ~mode_fullspeed),
        .o_toggle(next_toggle_a)
    );

    demo_toggle_pulse_tx u_prev_tx (
        .i_clk   (CLOCK_50),
        .i_rst_n (rst_n_sync),
        .i_pulse (prev_pulse & ~mode_fullspeed),
        .o_toggle(prev_toggle_a)
    );

    demo_toggle_pulse_rx u_start_rx (
        .i_clk         (core_clk),
        .i_rst_n       (rst_n_sync),
        .i_toggle_async(start_toggle_a),
        .o_pulse       (start_pulse_a_core)
    );

    demo_toggle_pulse_rx u_next_rx (
        .i_clk         (core_clk),
        .i_rst_n       (rst_n_sync),
        .i_toggle_async(next_toggle_a),
        .o_pulse       (next_pulse_a_core)
    );

    demo_toggle_pulse_rx u_prev_rx (
        .i_clk         (core_clk),
        .i_rst_n       (rst_n_sync),
        .i_toggle_async(prev_toggle_a),
        .o_pulse       (prev_pulse_a_core)
    );

    assign start_evt_core = mode_fullspeed ? start_pulse : start_pulse_a_core;
    assign next_evt_core  = mode_fullspeed ? next_pulse  : next_pulse_a_core;
    assign prev_evt_core  = mode_fullspeed ? prev_pulse  : prev_pulse_a_core;

    assign start_frame_cmd = start_evt_core & ~core_frame_done;
    assign next_frame_cmd  = start_evt_core &  core_frame_done;
    assign run_frame_cmd   = start_frame_cmd | next_frame_cmd;
    ...
    pure_quat_conv2d_3x3_core #(
        ...
        .i_clk            (core_clk),
        .i_rst_n          (rst_n_sync),
        .i_start          (start_frame_cmd),
        .i_next_frame     (next_frame_cmd),
    ...
    assign lane_sel_valid = lane_sel_valid_raw & core_frame_done;
    ...
    demo_readback_ctrl #(
        ...
        .i_next_pulse     (next_evt_core),
        .i_prev_pulse     (prev_evt_core),
